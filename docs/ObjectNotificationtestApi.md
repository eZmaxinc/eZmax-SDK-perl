...
> NotificationtestGetElementsV2Response notificationtest_get_elements_v2(pki_notificationtest_id => $pki_notificationtest_id)
...
eval {
    my $result = $api_instance->notificationtest_get_elements_v2(pki_notificationtest_id => $pki_notificationtest_id);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling ObjectNotificationtestApi->notificationtest_get_elements_v2: $@\n";
}
```
