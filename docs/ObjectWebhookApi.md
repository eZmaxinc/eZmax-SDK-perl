...
> WebhookGetObjectV1Response webhook_get_object_v1(pki_webhook_id => $pki_webhook_id)
...
eval {
    my $result = $api_instance->webhook_get_object_v1(pki_webhook_id => $pki_webhook_id);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling ObjectWebhookApi->webhook_get_object_v1: $@\n";
}
```
