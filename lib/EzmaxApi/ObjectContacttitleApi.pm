...
#
package EzmaxApi::ObjectContacttitleApi;
...
#
# @param string $s_selector The type of Contacttitles to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    's_selector' => {
        data_type => 'string',
        description => 'The type of Contacttitles to return',
        required => '1',
    },
    's_query' => {
        data_type => 'string',
    ...
    __PACKAGE__->method_documentation->{ 'contacttitle_get_autocomplete_v2' } = {
        summary => 'Retrieve Contacttitles and IDs',
        params => $params,
...
#
sub contacttitle_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling contacttitle_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/contacttitle/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('ContacttitleGetAutocompleteV2Response', $response);
