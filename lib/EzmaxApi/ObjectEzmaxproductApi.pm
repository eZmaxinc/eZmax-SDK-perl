...
#
package EzmaxApi::ObjectEzmaxproductApi;
...
#
# @param string $s_selector The type of Ezmaxproducts to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'ezmaxproduct_get_autocomplete_v2' } = {
...
#
sub ezmaxproduct_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling ezmaxproduct_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/ezmaxproduct/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('EzmaxproductGetAutocompleteV2Response', $response);
