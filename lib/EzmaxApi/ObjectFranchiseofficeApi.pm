...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'franchiseoffice_get_autocomplete_v2' } = {
        summary => 'Retrieve Franchiseoffices and IDs',
        params => $params,
...
#
sub franchiseoffice_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling franchiseoffice_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/franchiseoffice/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('FranchiseofficeGetAutocompleteV2Response', $response);
    return $_response_object;
}
