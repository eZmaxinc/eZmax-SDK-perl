...
sub new {
    ...
    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'EzmaxApi::Configuration' ) {
        $config = $_[0];
    } else {
        $config = EzmaxApi::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("EzmaxApi::Object::$class")->new;
        if (ref $data eq "HASH") {
