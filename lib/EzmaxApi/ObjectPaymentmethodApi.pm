...
#
package EzmaxApi::ObjectPaymentmethodApi;
...
#
# @param string $s_selector The type of Paymentmethods to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    's_selector' => {
        data_type => 'string',
        description => 'The type of Paymentmethods to return',
        required => '1',
    },
    'e_filter_active' => {
        data_type => 'string',
    ...
    __PACKAGE__->method_documentation->{ 'paymentmethod_get_autocomplete_v2' } = {
...
#
sub paymentmethod_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling paymentmethod_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/paymentmethod/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('PaymentmethodGetAutocompleteV2Response', $response);
