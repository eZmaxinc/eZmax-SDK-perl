...
#
package EzmaxApi::ObjectUserlogintypeApi;
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'userlogintype_get_autocomplete_v2' } = {
...
#
sub userlogintype_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling userlogintype_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/userlogintype/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('UserlogintypeGetAutocompleteV2Response', $response);
