...
#
package EzmaxApi::ObjectCompanyApi;
...
#
# @param string $s_selector The type of Companys to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'company_get_autocomplete_v2' } = {
...
#
sub company_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling company_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/company/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('CompanyGetAutocompleteV2Response', $response);
