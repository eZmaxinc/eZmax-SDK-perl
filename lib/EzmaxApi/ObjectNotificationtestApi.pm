...
# @param int $pki_notificationtest_id  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'notificationtest_get_elements_v2' } = {
...
#
sub notificationtest_get_elements_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 'pki_notificationtest_id' is set
    unless (exists $args{'pki_notificationtest_id'}) {
      croak("Missing the required parameter 'pki_notificationtest_id' when calling notificationtest_get_elements_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/notificationtest/{pkiNotificationtestID}/getElements';
    ...
    my $_response_object = $self->{api_client}->deserialize('NotificationtestGetElementsV2Response', $response);
