...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'billingentityinternal_get_autocomplete_v1' } = {
...
#
sub billingentityinternal_get_autocomplete_v1 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling billingentityinternal_get_autocomplete_v1");
    }

    # parse inputs
    my $_resource_path = '/1/object/billingentityinternal/getAutocomplete/{sSelector}';
