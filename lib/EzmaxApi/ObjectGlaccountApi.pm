...
#
package EzmaxApi::ObjectGlaccountApi;
...
#
# @param string $s_selector The type of Glaccounts to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    's_selector' => {
        data_type => 'string',
        description => 'The type of Glaccounts to return',
        required => '1',
    },
    'e_filter_active' => {
        data_type => 'string',
    ...
    __PACKAGE__->method_documentation->{ 'glaccount_get_autocomplete_v2' } = {
        summary => 'Retrieve Glaccounts and IDs',
        params => $params,
...
#
sub glaccount_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling glaccount_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/glaccount/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('GlaccountGetAutocompleteV2Response', $response);
