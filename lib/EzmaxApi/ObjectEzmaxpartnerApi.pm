...
#
package EzmaxApi::ObjectEzmaxpartnerApi;
...
# @param string $s_filter  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'ezmaxpartner_get_custom_developpers_v1' } = {
...
#
sub ezmaxpartner_get_custom_developpers_v1 {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/1/object/ezmaxpartner/getCustomDeveloppers';
    ...
    my $_response_object = $self->{api_client}->deserialize('EzmaxpartnerGetCustomDeveloppersV1Response', $response);
...
#
# @param int $pki_ezmaxpartner_id The unique ID of the Ezmaxpartner (required)
{
    my $params = {
    'pki_ezmaxpartner_id' => {
        data_type => 'int',
    ...
    __PACKAGE__->method_documentation->{ 'ezmaxpartner_get_object_v2' } = {
...
#
sub ezmaxpartner_get_object_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 'pki_ezmaxpartner_id' is set
    unless (exists $args{'pki_ezmaxpartner_id'}) {
      croak("Missing the required parameter 'pki_ezmaxpartner_id' when calling ezmaxpartner_get_object_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/ezmaxpartner/{pkiEzmaxpartnerID}';
    ...
    # path params
    if ( exists $args{'pki_ezmaxpartner_id'}) {
        my $_base_variable = "{" . "pkiEzmaxpartnerID" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'pki_ezmaxpartner_id'});
    ...
    my $_response_object = $self->{api_client}->deserialize('EzmaxpartnerGetObjectV2Response', $response);
    return $_response_object;
}
