...
#
package EzmaxApi::ObjectModuleApi;
...
#
# @param string $s_selector The type of Modules to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    's_selector' => {
        data_type => 'string',
        description => 'The type of Modules to return',
        required => '1',
    },
    'e_filter_active' => {
        data_type => 'string',
    ...
    __PACKAGE__->method_documentation->{ 'module_get_autocomplete_v2' } = {
...
#
sub module_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling module_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/module/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('ModuleGetAutocompleteV2Response', $response);
