...
# @param int $pki_variableexpense_id The unique ID of the Variableexpense (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'variableexpense_delete_object_v1' } = {
        summary => 'Delete an existing Variableexpense',
        params => $params,
        returns => 'VariableexpenseDeleteObjectV1Response',
        };
}
# @return VariableexpenseDeleteObjectV1Response
#
sub variableexpense_delete_object_v1 {
    my ($self, %args) = @_;

    # verify the required parameter 'pki_variableexpense_id' is set
    unless (exists $args{'pki_variableexpense_id'}) {
      croak("Missing the required parameter 'pki_variableexpense_id' when calling variableexpense_delete_object_v1");
    }
    ...
    my $_response_object = $self->{api_client}->deserialize('VariableexpenseDeleteObjectV1Response', $response);
    return $_response_object;
}
