...
#
package EzmaxApi::ObjectLanguageApi;
...
#
# @param string $s_selector The type of Languages to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'language_get_autocomplete_v2' } = {
...
#
sub language_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling language_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/language/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('LanguageGetAutocompleteV2Response', $response);
