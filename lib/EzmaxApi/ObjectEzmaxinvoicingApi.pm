...
# @param int $pki_ezmaxinvoicing_id  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'ezmaxinvoicing_get_object_v2' } = {
        summary => 'Retrieve an existing Ezmaxinvoicing',
        params => $params,
...
#
sub ezmaxinvoicing_get_object_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 'pki_ezmaxinvoicing_id' is set
    unless (exists $args{'pki_ezmaxinvoicing_id'}) {
      croak("Missing the required parameter 'pki_ezmaxinvoicing_id' when calling ezmaxinvoicing_get_object_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/ezmaxinvoicing/{pkiEzmaxinvoicingID}';
    ...
    my $_response_object = $self->{api_client}->deserialize('EzmaxinvoicingGetObjectV2Response', $response);
    return $_response_object;
}
