...
#
package EzmaxApi::ObjectEmailtypeApi;
...
#
# @param string $s_selector The type of Emailtypes to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'emailtype_get_autocomplete_v2' } = {
...
#
sub emailtype_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling emailtype_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/emailtype/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('EmailtypeGetAutocompleteV2Response', $response);
