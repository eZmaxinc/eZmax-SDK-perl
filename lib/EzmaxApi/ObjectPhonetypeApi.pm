...
#
package EzmaxApi::ObjectPhonetypeApi;
...
#
# @param string $s_selector The type of Phonetypes to return (required)
...
# @param HeaderAcceptLanguage $accept_language  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'phonetype_get_autocomplete_v2' } = {
...
#
sub phonetype_get_autocomplete_v2 {
    my ($self, %args) = @_;

    # verify the required parameter 's_selector' is set
    unless (exists $args{'s_selector'}) {
      croak("Missing the required parameter 's_selector' when calling phonetype_get_autocomplete_v2");
    }

    # parse inputs
    my $_resource_path = '/2/object/phonetype/getAutocomplete/{sSelector}';
    ...
    my $_response_object = $self->{api_client}->deserialize('PhonetypeGetAutocompleteV2Response', $response);
